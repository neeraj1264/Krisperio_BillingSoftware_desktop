...
/**
 * Open WhatsApp with the given phone (optional) and message text.
 * - For 10-digit numbers we assume India and prefix 91.
 * - For other digits we use as-is.
 * - If phone is empty, opens WhatsApp Web with message only.
 */
...
export function formatOrderMessage({
  ...
  list.forEach((p, i) => {
    ...
    itemTotal += total;
    lines.push(`${i + 1}. ${qty} x ${p.name}${size} = ₹${total.toFixed(2)}`);
  });
